use {
    ...
    huione_client::rpc_client::RpcClient,
    huione_sdk::{
        clock::UnixTimestamp,
    ...
    hpl_feature_proposal::state::{AcceptanceCriteria, FeatureProposal},
    std::{
        collections::HashMap,
        fs::File,
        io::Write,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            SubCommand::with_name("propose")
                ...
                .arg(
                    Arg::with_name("distribution_file")
                        ...
                        .help("Allocations CSV file for use with huione-tokens"),
                )
                .arg(
    ...
    match (sub_command, sub_matches) {
        ("address", Some(arg_matches)) => {
            ...
            println!(
                "Feature Id: {}",
                hpl_feature_proposal::get_feature_id_address(&feature_proposal_address)
            );
            println!(
                "Token Mint Address: {}",
                hpl_feature_proposal::get_mint_address(&feature_proposal_address)
            );
            println!(
                "Acceptance Token Address: {}",
                hpl_feature_proposal::get_acceptance_token_address(&feature_proposal_address)
            );
...
fn process_propose(
    ...
    let distributor_token_address =
        hpl_feature_proposal::get_distributor_token_address(&feature_proposal_keypair.pubkey());
    let feature_id_address =
        hpl_feature_proposal::get_feature_id_address(&feature_proposal_keypair.pubkey());
    let acceptance_token_address =
        hpl_feature_proposal::get_acceptance_token_address(&feature_proposal_keypair.pubkey());
    let mint_address = hpl_feature_proposal::get_mint_address(&feature_proposal_keypair.pubkey());
    ...
    println!(
        "Tokens to be minted: {}",
        hpl_feature_proposal::amount_to_ui_amount(tokens_to_mint)
    );
    println!(
        "Tokens required for acceptance: {} ({}%)",
        hpl_feature_proposal::amount_to_ui_amount(tokens_required),
        percent_stake_required
    );
    ...
    let mut transaction = Transaction::new_with_payer(
        &[hpl_feature_proposal::instruction::propose(
    ...
    println!(
        "    $ huione-tokens distribute-hpl-tokens \
                  --from {} \
                  --input-csv {} \
                  --db-path db.{} \
                  --fee-payer ~/.config/huione/id.json \
    ...
    println!(
        "    $ huione-tokens hpl-token-balances \
                 --mint {} --input-csv {}",
    ...
    println!(
        "    $ hpl-token --owner ~/validator-keypair.json accounts {}",
        mint_address
    );
    println!("and then submit their vote by running:");
    println!(
        "    $ hpl-token --owner ~/validator-keypair.json transfer <TOKEN_ACCOUNT_ADDRESS> ALL {}",
        acceptance_token_address
    );
    println!();
    println!("Periodically the votes must be tallied by running:");
    println!(
        "  $ hpl-feature-proposal tally {}",
        feature_proposal_keypair.pubkey()
    );
...
fn process_tally(
    ...
    let feature_id_address = hpl_feature_proposal::get_feature_id_address(feature_proposal_address);
    let acceptance_token_address =
        hpl_feature_proposal::get_acceptance_token_address(feature_proposal_address);
    ...
    match feature_proposal {
        ...
        FeatureProposal::Pending(acceptance_criteria) => {
            ...
            println!(
                ...
                hpl_feature_proposal::amount_to_ui_amount(acceptance_criteria.tokens_required)
            );
            println!(
                "{} tokens have been received",
                hpl_feature_proposal::amount_to_ui_amount(acceptance_token_balance)
            );
            println!(
                "Proposal will expire at {}",
    ...
    let mut transaction = Transaction::new_with_payer(
        &[hpl_feature_proposal::instruction::tally(
            feature_proposal_address,
        )],
        Some(&config.keypair.pubkey()),
    );
