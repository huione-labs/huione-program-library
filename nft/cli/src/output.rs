...
use huione_cli_output::{display::writeln_name_value, OutputFormat, QuietDisplay, VerboseDisplay};
use std::fmt::{self, Display};
use huione_account_decoder::parse_nft::{UiNFTAccount, UiAccountState};
        ...
        T: Serialize + Display + QuietDisplay + VerboseDisplay,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f)?;
        writeln_name_value(f, "Address: ", &self.address)?;
        writeln_name_value(f, "token_uri: ", &self.nft_uri)?;
        Display::fmt(&self.transaction_data, f)
    }
}
        ...
        T: Serialize + Display + QuietDisplay + VerboseDisplay,
{
    fn write_str(&self, w: &mut dyn std::fmt::Write) -> std::fmt::Result {
        writeln!(w)?;
        writeln_name_value(w, "Address: ", &self.address)?;
        writeln_name_value(w, "nft_uri: ", &self.nft_uri)?;
        QuietDisplay::write_str(&self.transaction_data, w)
    }
}
        ...
        T: Serialize + Display + QuietDisplay + VerboseDisplay,
{
    fn write_str(&self, w: &mut dyn std::fmt::Write) -> std::fmt::Result {
        writeln!(w)?;
        writeln_name_value(w, "Address: ", &self.address)?;
        writeln_name_value(w, "token_uri: ", &self.nft_uri)?;
        VerboseDisplay::write_str(&self.transaction_data, w)
    }
}
...
impl fmt::Display for CliDisplayNftInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln_name_value(f, "NFT Collection:", &self.account.collection.to_string())?;
        writeln_name_value(f, "NFT Name:", &self.account.name)?;
        writeln_name_value(f, "NFT Symbol:", &self.account.symbol)?;
        writeln_name_value(f, "NFT Owner:", &self.account.owner.to_string())?;
        writeln_name_value(f, "NFT State:", &self.account.state)?;
        writeln_name_value(f, "NFT Id:", &self.account.nft_id.to_string())?;
        writeln_name_value(f, "NFT Author:", &self.account.author.to_string())?;
        writeln_name_value(f, "NFT Proposal:", &self.account.proposal.to_string())?;
        writeln_name_value(f, "NFT Uri:", &self.account.nft_uri)?;
        writeln_name_value(f, "NFT Close Authority:", &close_authority)?;
        writeln_name_value(f, "NFT Timestamp:", &self.account.timestamp.to_string())?;
        Ok(())
    }
}
...
impl fmt::Display for CliNftAccount {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln_name_value(
            f,
            "NftId:",
            &self.account.nft_id.to_string(),
        )?;
        let collection = format!(
            "{}",
            self.account.collection,
        );
        writeln_name_value(f, "Collection:", &collection)?;
        writeln_name_value(f, "Author:", &self.account.author)?;
        writeln_name_value(f, "Proposal:", &self.account.proposal)?;
        writeln_name_value(f, "NftTimestamp:", &format!("{:?}", self.account.mint_timestamp))?;
        writeln_name_value(f, "Owner:", &self.account.owner)?;
        writeln_name_value(f, "State:", &format!("{:?}", self.account.state))?;
        writeln_name_value(f, "NftUri:", &self.account.nft_uri)?;
            ...
            for account in accounts_list {
                ...
                if self.token_is_some {
                    writeln!(
                        ...
                        "{:<44}  {:<3$}{}",
                        account.address,
                        account.account.nft_id.to_string(),
                        maybe_frozen,
