...
use crate::{
    error::TokenError,
    instruction::{MintNftArgs, TokenInstruction},
    state::{AccountState, MetaAccount, NftMint},
};
use num_traits::FromPrimitive;
use huione_program::{account_info::{next_account_info, AccountInfo}, decode_error::DecodeError, entrypoint::ProgramResult, msg, program_error::{PrintProgramError, ProgramError}, program_memory::{huione_memcmp}, pubkey::{Pubkey, PUBKEY_BYTES}, system_instruction, sysvar::{rent::Rent, Sysvar}};
use huione_program::program::{invoke, invoke_signed};
use huione_program::program_memory::huione_memset;
use huione_program::program_pack::Pack;
...
impl Processor {
    ...
    fn _process_mint_to(
        ...
        let nft_collection_mint_authority_account_info = next_account_info(account_info_iter)?;
        let nft_author_account_info = next_account_info(account_info_iter)?;
        let nft_proposal_account_info = next_account_info(account_info_iter)?;
        let fee_payer_account_info = next_account_info(account_info_iter)?;
        let nft_collection_account_info = next_account_info(account_info_iter)?;
        let nft_account_account_info = next_account_info(account_info_iter)?;
        let system_program_info = next_account_info(account_info_iter)?;
        let rent_account_info = next_account_info(account_info_iter)?;
        let owner = if let Some(owner) = owner {
            owner
        } else {
            * nft_proposal_account_info.key
        };


        // 2、check authority
        if !Self::cmp_pubkeys(program_id, &nft_collection_account_info.owner) {
            return Err(TokenError::OwnerMismatch.into());
        }
        // will check init.
        let mut mint = NftMint::unpack(&nft_collection_account_info.data.borrow())?;
        if mint.supply == mint.total_supply {
            return Err(TokenError::AlreadyReachMaxMintNum.into());
        }

        if mint.mint_authority != *nft_collection_mint_authority_account_info.key {
        ...
        meta.collection = *nft_collection_account_info.key;
        meta.author = *nft_author_account_info.key;
        meta.proposal = *nft_proposal_account_info.key;
        meta.owner = owner;
        meta.close_authority = closer;
        meta.state = AccountState::Initialized;
        meta.nft_id = mint.supply + 1;
        meta.mint_timestamp = time_stamp;
        meta.nft_uri = nft_uri.clone();
        ...
        let required_lamports = rent
            ...
            .max(1)
            .saturating_sub(nft_account_account_info.lamports());

        if required_lamports > 0 {
            msg!("Transfer {} lamports to the new account", required_lamports);
            invoke(
                &system_instruction::transfer(fee_payer_account_info.key, nft_account_account_info.key, required_lamports),
                &[
                    fee_payer_account_info.clone(),
                    nft_account_account_info.clone(),
                    system_program_info.clone(),
                ],
            )?;
        }
        ...
        let index = time_stamp.to_le_bytes();
        let signer_seeds = &[
            index.as_ref(),
            program_id.as_ref(),
            nft_collection_account_info.key.as_ref()
        ];
        let (_, metadata_bump_seed) =
            Pubkey::find_program_address(signer_seeds, &program_id);
        let metadata_signer_seeds = &[
        ...
        let allocate_accounts = &[nft_account_account_info.clone(), system_program_info.clone()];

        msg!("Allocate space for the account");
        invoke_signed(
            &system_instruction::allocate(nft_account_account_info.key, MAX_META_DATA_SIZE as u64),
            allocate_accounts,
            &[metadata_signer_seeds]
        )?;
        ...
        msg!("Assign the account to the owning program");
        invoke_signed(
            &system_instruction::assign(nft_account_account_info.key, &program_id),
            allocate_accounts,
            &[metadata_signer_seeds],
        )?;
        ...
        MetaAccount::pack(meta, &mut nft_account_account_info.data.borrow_mut())?;
        ...
        mint.supply += 1;
        NftMint::pack(mint, &mut nft_collection_account_info.data.borrow_mut())?;
    ...
    pub fn process_mint_to(
        ...
        Self::_process_mint_to(program_id, accounts, args.nft_owner,args.nft_closer,args.nft_uri,args.time_stamp,true)
    }
    ...
    pub fn process_transfer(
        ...
        msg!("transfer_nft [{}] from [{}] to [{}]",nft_account_info.key, source_account_info.key, destination_account_info.key);
    ...
    pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], input: &[u8]) -> ProgramResult {
        ...
        match instruction {
            ...
            TokenInstruction::MintTo (args) => {
                msg!("Instruction: InitializeAccount");
                Self::process_mint_to(program_id, accounts, args)
            }
            ...
            TokenInstruction::Freeze => {
                ...
                if !Self::cmp_pubkeys(&meta.collection, &mint_account.key) {
            ...
            TokenInstruction::Burn => {
                ...
                **nft_account_info.lamports.borrow_mut() = 0;
                huione_memset(*nft_account_info.data.borrow_mut(), 0, MetaAccount::LEN);
                Ok(())
            }
    ...
    pub fn cmp_pubkeys(a: &Pubkey, b: &Pubkey) -> bool {
        huione_memcmp(a.as_ref(), b.as_ref(), PUBKEY_BYTES) == 0
    }
...
mod tests {
    ...
    use huione_program::instruction::Instruction;
    use huione_program::program_error::ProgramError;
    use huione_program::program_pack::Pack;
    use huione_program::pubkey::Pubkey;
    use huione_sdk::account::{create_is_signer_account_infos};
    ...
    use huione_sdk::account::Account as HuiOneAccount;
    use crate::error::TokenError;

    fn do_process_instruction(
        instruction: Instruction,
        accounts: Vec<&mut HuiOneAccount>,
    ) -> ProgramResult {
    ...
    fn test_transfer() {
        ...
        let mut nft_account = HuiOneAccount::new(1000, MAX_META_DATA_SIZE, &program_id);
        nft_account.data = nft_account_data.to_vec();

        let mut from_account = HuiOneAccount::default();
        let mut to_account = HuiOneAccount::default();
    ...
    fn test_freeze() {
        ...
        let mut mint_account = HuiOneAccount::new(10, MINT_SIZE, &program_id);
        ...
        let mut authority_account = HuiOneAccount::default();
        ...
        nft_account_data_obj.collection = mint_account_puk;
        MetaAccount::pack(nft_account_data_obj.clone(), &mut nft_account_data);
        nft_account.data = nft_account_data.to_vec();
    ...
    fn test_set_authority() {
        ...
        let mut owner_account = HuiOneAccount::default();
