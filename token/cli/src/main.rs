...
use console::Emoji;
use hpl_associated_token_account::*;
use hpl_token::{
    self,
    instruction::*,
    native_mint,
...
use huione_cli_output::{display::println_name_value, return_signers, OutputFormat};
use huione_client::{
...
fn command_create_token(
    ...
    let (mint_info, _) =
        Pubkey::find_program_address(&[b"MintMeta", &token.to_bytes()], &hpl_token::id());
    ...
    let mut instructions = vec![
        ...
        create_mint_meta_account(&hpl_token::id(), &config.fee_payer, &token, &mint_info)?,
        initialize_mint(
            &hpl_token::id(),
            &token,
            &authority,
    ...
    if let Some(text) = memo {
        instructions.push(hpl_memo::build_memo(text.as_bytes(), &[&config.fee_payer]));
    }
    Ok(Some((
    ...
    let (account, system_account_ok, instructions) = if let Some(account) = maybe_account {
        ...
        (
            ...
            vec![
                ...
                initialize_account(&hpl_token::id(), &account, &token, &owner)?,
            ],
        )
    } else {
        ...
        if let Ok(mint) = Mint::unpack(&target_account.data) {
            match authority_type {
                AuthorityType::AccountOwner | AuthorityType::CloseAccount => Err(format!(
                    "Authority type `{}` not supported for HPL Token mints",
                    auth_str
                )),
                ...
                AuthorityType::UpdateMeta => {
                    ...
                    if account == hpl_token::native_mint::id() {
                        Err("Unsupported on native mint account ".to_string())
                    }else{
                        let (mint_meta, _) =
                        Pubkey::find_program_address(&[b"MintMeta", &account.to_bytes()], &hpl_token::id());
            ...
            match authority_type {
                AuthorityType::MintTokens | AuthorityType::FreezeAccount | AuthorityType::UpdateMeta => Err(format!(
                    "Authority type `{}` not supported for HPL Token accounts",
                    auth_str
                )),
...
fn command_transfer(
    ...
    let maybe_transfer_balance =
        ui_amount.map(|ui_amount| {
            ...
            hpl_token::ui_amount_to_amount(inte,frac, decimals)
        });
    let transfer_balance = if !config.sign_only {
        ...
        println!(
            "Transfer {} tokens\n  Sender: {}\n  Recipient: {}",
            hpl_token::amount_to_ui_amount(transfer_balance, decimals),
            sender,
            recipient
        );
    ...
    let recipient_is_token_account = if !config.sign_only {
        let recipient_account_info = config
            ...
            .value
            .map(|account| account.owner == hpl_token::id() && account.data.len() == Account::LEN);
        ...
        let needs_funding = if !config.sign_only {
                ...
                .value
            {
                if recipient_token_account_data.owner == system_program::id() {
                    true
                } else if recipient_token_account_data.owner == hpl_token::id() {
                    false
                } else {
                    return Err(
            ...
            if fund_recipient {
                if !config.sign_only {
                    minimum_balance_for_rent_exemption += config
                        ...
                        .get_minimum_balance_for_rent_exemption(Account::LEN)?;
                    println!(
                        "  Funding recipient: {} ({} HC)",
                        recipient_token_account,
                        lamports_to_hc(minimum_balance_for_rent_exemption)
                    );
                }
...
fn command_burn(
    ...
    let (inte,frac) = parse_amount(ui_amount.as_str());
    let amount = hpl_token::ui_amount_to_amount(inte,frac, decimals);
...
fn command_wrap(
    ...
    wrapped_huione_account: Option<Pubkey>,
) -> CommandResult {
    let lamports = hc_to_lamports(&hc);

    if lamports == 0 {
       return Err(format!("Wrap HC amount too small ,  HC decimals is 9.").into());
    }

    let instructions = if let Some(wrapped_huione_account) = wrapped_huione_account {
        println!("Wrapping {} HC into {}", hc, wrapped_huione_account);
        vec![
            system_instruction::create_account(
                &wallet_address,
                &wrapped_huione_account,
                lamports,
                Account::LEN as u64,
                &hpl_token::id(),
            ),
        ...
        println!("Wrapping {} HC into {}", hc, account);
        vec![
    ...
    if !config.sign_only {
        ...
        if lamports == 0 {
            if use_associated_account {
                return Err("No wrapped HC in associated account; did you mean to specify an auxiliary address?".to_string().into());
            } else {
                return Err(format!("No wrapped HC in {}", address).into());
            }
        }
        println!("  Amount: {} HC", lamports_to_hc(lamports),);
    }
...
fn command_accounts(config: &Config, token: Option<Pubkey>, owner: Pubkey) -> CommandResult {
    ...
    let accounts = config.rpc_client.get_token_accounts_by_owner(
        ...
        match token {
            ...
            None => TokenAccountsFilter::ProgramId(hpl_token::id()),
        },
    )?;
    ...
    if gc_alert {
        println!();
        println!("* Please run `hpl-token gc` to clean up Aux accounts");
    }
    Ok(None)
}
...
fn command_gc(config: &Config, owner: Pubkey) -> CommandResult {
    ...
    let accounts = config
        .rpc_client
        .get_token_accounts_by_owner(&owner, TokenAccountsFilter::ProgramId(hpl_token::id()))?;
    if accounts.is_empty() {
    ...
    for keyed_account in accounts {
        if let UiAccountData::Json(parsed_account) = keyed_account.account.data {
            if parsed_account.program == "hpl-token" {
...
fn command_sync_native(native_account_address: Pubkey) -> CommandResult {
    let instructions = vec![sync_native(&hpl_token::id(), &native_account_address)?];
    Ok(Some((0, vec![instructions])))
}

fn command_token_info(config: &Config, address: Pubkey) -> CommandResult {
    let (mut mint_info, _) =
        Pubkey::find_program_address(&[b"MintMeta", &address.to_bytes()], &hpl_token::id());

    if address == hpl_token::native_mint::id() {
        mint_info = hpl_token::native_mint_info::id();
    }
...
fn main() {
    ...
    let app_matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("json_rpc_url")
                ...
                .help(
                    "URL for HC's JSON RPC or moniker (or their first letter): \
        ...
        .subcommand(
            SubCommand::with_name("wrap")
                .about("Wrap native HC in a HC token account")
                .arg(
                    Arg::with_name("amount")
                        .validator(is_amount)
                        .value_name("AMOUNT")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which will have its native HC wrapped. \
                             This wallet will be assigned as the owner of the wrapped HC token account. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
                .arg(
                    Arg::with_name("create_aux_account")
                        ...
                        .help("Wrap HC in an auxillary account instead of associated token account"),
                )
                .nonce_args(true)
                .offline_args(),
        )
        .subcommand(
            SubCommand::with_name("unwrap")
                .about("Unwrap a HC token account")
                .arg(
                    Arg::with_name("address")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ACCOUNT_ADDRESS")
                ...
                .arg(
                    Arg::with_name("wallet_keypair")
                        ...
                        .takes_value(true)
                        .help(
                            "Specify the keypair for the wallet which owns the wrapped HC. \
                             This wallet will receive the unwrapped HC. \
                             This may be a keypair file or the ASK keyword. \
                             Defaults to the client keypair."
                        ),
                )
            ...
            SubCommand::with_name("close")
                ...
                .arg(
                    Arg::with_name("recipient")
                        ...
                        .takes_value(true)
                        .help("The address of the account to receive remaining HC [default: --owner]"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("account-info")
                .about("Query details of an HPL Token account by address")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
        ...
        .subcommand(
            SubCommand::with_name("multisig-info")
                .about("Query details about and HPL Token multisig account by address")
                .arg(
                    Arg::with_name("address")
                    .validator(is_valid_pubkey)
                    ...
                    .index(1)
                    .required(true)
                    .help("The address of the HPL Token multisig account to query"),
                ),
        )
        .subcommand(
        ...
        .subcommand(
            SubCommand::with_name("sync-native")
                .about("Sync a native HC token account to its underlying lamports")
                .arg(
                    owner_address_arg()
                        .index(1)
                        .conflicts_with("address")
        ...
        .subcommand(
            SubCommand::with_name("token-info")
                .about("Query details of an HPL Token")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
        ...
        .subcommand(
            SubCommand::with_name("update-token-info")
                .about("Update info of an HPL Token")
                .arg(
                    Arg::with_name("token")
                        .validator(is_valid_pubkey)
                        .value_name("TOKEN_ADDRESS")
    ...
    huione_logger::setup_with_default("hc=info");
