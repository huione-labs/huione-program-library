...
pub enum TokenInstruction {
    ...
    /// Close an account by transferring all its HC to the destination account.
    ...
    /// Given a wrapped / native token account (a token account containing HC)
    /// updates its amount field based on the account's underlying `lamports`.
    /// This is useful if a non-wrapped HC account uses `system_instruction::transfer`
