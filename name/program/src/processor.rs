use std::ops::{Div};
use std::slice::Iter;
use huione_program::account_info::{AccountInfo, next_account_info};
use huione_program::entrypoint::ProgramResult;
use huione_program::hash::{hashv};
use huione_program::{clock, msg, system_instruction, system_program};
...
impl Processor {
    ...
    fn _update_domain_resolve_value(program_id: &Pubkey, accounts_iter: &mut Iter<AccountInfo>, new_value: Vec<u8>, domain_name: String) -> ProgramResult{
        ...
        if domain_resolve_data_obj.value.is_some() {
            ...
            **old_address_resolve_account.lamports.borrow_mut() = 0;
            ...
            huione_memset(*old_address_resolve_account.data.borrow_mut(), 0, data_len);
        }
    ...
    fn _unbind_address(program_id: &Pubkey, accounts_iter: &mut Iter<AccountInfo>) -> ProgramResult{
        ...
        **address_resolve_account.lamports.borrow_mut() = 0;
        ...
        huione_memset(*address_resolve_account.data.borrow_mut(), 0, data_len);
    ...
    fn _close_domain_resolve_account(program_id: &Pubkey, accounts_iter: &mut Iter<AccountInfo>) -> ProgramResult{
        ...
        if domain_resolve_data_obj.value.is_some() {
            ...
            // Delete the address resolution account
            **address_resolve_account.lamports.borrow_mut() = 0;
            huione_memset(*address_resolve_account.data.borrow_mut(), 0, address_data_len);
        }
        ...
        **domain_resolve_account.lamports.borrow_mut() = 0;
        huione_memset(*domain_resolve_account.data.borrow_mut(), 0, resolve_data_len);
    ...
    /// through 1 huione -> target token price,
    /// get 1 target token -> huione price,
    /// decimal is 9
    fn token_to_hc(token_price: u128) -> u128 {
        let rate = HC_BASE.div(token_price as f64);
        let ret = (HC_BASE * rate) as u128;
        ret
    }
...
mod tests {
    ...
    use huione_sdk::account::{Account as HuiOneAccount, create_is_signer_account_infos};
    use crate::error::NameError;
    ...
    fn test_transfer() {
        ...
        let mut domain_account_data_obj = DomainAccount{
            ...
            max_space: 32,
            fee_index: 0,
            domain_name: "fb.huione".to_string()
        };

        let domain_account_data = borsh::to_vec(&domain_account_data_obj).unwrap();
        let mut domain_account = HuiOneAccount::new(1000, domain_account_data.len(), &program_id);
        domain_account.data = domain_account_data;
        // let mut rent_sysvar = rent_sysvar();
        let mut domain_owner_account = HuiOneAccount::default();
        let mut receipt_account = HuiOneAccount::default();
    ...
    fn test_unbind_address() {
        ...
        let mut domain_resolve_account = HuiOneAccount::new(1000, domain_resolve_account_data.len(), &program_id);
        ...
        let mut parent_account = HuiOneAccount::new(1000, parent_account_data.len(), &program_id);
        parent_account.data = parent_account_data;
        let mut address_resolve_account = HuiOneAccount::new(1000, 43, &program_id);
        ...
        let mut top_domain_account = HuiOneAccount::new(1000, top_domain_account_data.len(), &program_id);
