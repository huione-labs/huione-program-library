...
pub enum NameInstruction {
    ...
    CreateDomain {
        /// Domain name, for example: aaaa.huione
        domain_name: String,
    },
    ...
    CreateRareDomain {
        /// Rare Domain name, eg: aaaa.huione.
        domain_name: String,
    },
    ...
    CreateDomainResolveAccount {
        /// Resolve Domain name.  For example: aaaa.huione or bbb.aaaa.huione or ccc.bbb.aaaa.huione
        domain_name: String,

        /// The value of the domain resolve account, for .huione it's huione account address.
        value: Vec<u8>,
    },
    ...
    UpdateDomainResolveAccount {
        /// address value, for .huione its huione account address
        new_value: Vec<u8>,
...
pub fn is_valid_domain(input: &str) -> bool {
    ...
    let inhuione_split = input.split(".").collect::<Vec<&str>>();
    let inhuione_split_len = inhuione_split.len();
    if inhuione_split_len< 2 || inhuione_split_len > 4 {
        return false
    }
    for (index, child_domain) in inhuione_split.iter().enumerate() {
        if child_domain.len() > 32 {
            return false
        }
        // .a.huione、..huione、. 、huione. invalid, but .huione is valid
        if child_domain.len() == 0 && inhuione_split_len != 2{
            return false
        }
        // top domain must include alphabet
        if index == inhuione_split_len - 1  && !is_lower_alph(child_domain) {
            return false
        }
    }
...
mod tests {
    ...
    fn test_is_valid_domain() {
        let valid_domain1 = ".huione";
        let valid_domain2 = "abb.huione";
        let valid_domain3 = "ccc12.abb.huione";
        let valid_domain4 = "ga.ccc12.abb.huione";
        let invalid_domain1 = "AS.huione";
        let invalid_domain2 = ".AS.huione";
        let invalid_domain3 = "AS.huione.";
        let invalid_domain4 = "AS.huione.123";
        let invalid_domain5 = ".";
        let invalid_domain6 = "11.ga.ccc12.abb.huione";
