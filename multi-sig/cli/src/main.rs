#![allow(deprecated)] // TODO: Remove when SPL upgrades to huione 1.8
...
use huione_sdk::{commitment_config::CommitmentConfig, instruction::Instruction, message::Message, native_token::*, pubkey::Pubkey, signature::{Signer}, system_instruction, system_program, transaction::Transaction};
...
use huione_clap_utils::offline::{DUMP_TRANSACTION_MESSAGE, SIGN_ONLY_ARG};
...
mod output;
use output::*;
use hpl_sig::instruction::{create_add_signer, create_close_proposal_ins, create_multi_sig_account, create_proposal_account, create_remove_signer, create_set_new_threshold, create_vote_ins, verify};
use hpl_sig::state::{AccountState, MultiSigAccount, ProposalAccount};
...
fn main() -> Result<(), Error> {
    ...
    let default_program_id = hpl_sig::id().to_string();
